...
function removeDuplicates(list){
   let list1 = list.first();
   while( !list.isLast(list1)){
     let list2 = list1;
      while(!list.isLast(list2)){
        if(list1.element() == list.after(list2).element()){
            list.remove(list.after(list2));
        }
        else list2 = list.after(list2);
      }
      list1 = list.after(list1);
   }
}
...
function permutation(arr1,arr2){
    ...
    if(arr1.length == null && arr2.length == null){
        return true;
    }
...
console.log(removeDuplicates(tst2));
tst2.print();